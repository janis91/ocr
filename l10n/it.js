OC.L10N.register(
    ...
    "Text recognition for your images" : "Riconoscimento del testo per le tue immagini",
    "# Description\n\nNextcloud OCR (optical character recognition) processing for images with tesseract-js brings OCR capability to your Nextcloud.\nThe app uses tesseract-js in the browser in order to process images (png, jpeg, tiff, bmp) and saves the output PDF file to the source folder in nextcloud. That for example enables you to search in it.\n\n## Prerequisites, Requirements and Dependencies\nThe OCR app has some prerequisites:\n\n - [Nextcloud 16 and up](https://nextcloud.com/)\n\n - Only supported on latest modern web browsers (Chrome, Edge, Firefox, Opera, Safari*)\n\n - Tesseract traineddata needs about 200 MB space on your server (will be installed automatically).\n\n\n_* On Safari there is currently a problem with the [Content-Security-Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP), that requires an Administrator to set the 'script-src' to 'unsafe-eval' such that the app works properly. Because this is quite insecure the app itself does not set it and recommends to decide that on your own risk (please make sure, that you know what CSP is and what e.g. unsafe-eval causes)._\n\n## Installation\nInstall the app from the [Nextcloud App Store](https://apps.nextcloud.com/apps/ocr) or download the release package from github (**NOT** the sources) and place the content in **nextcloud/apps/ocr/**.\n\n## Disclaimer\nThe software is distributed on an \"AS IS\" BASIS, WITHOUT WARRANTIES OR\nCONDITIONS OF ANY KIND, either express or implied.\n\n## Note\nThe version 3 and earlier versions are not supported/maintained anymore by the author. So for asynchronous background processing please fork the repository and use the \"not-maintained\" branch to work on improvements. The author wasn't able to support it because of too much effort.\nMoreover this project is based on a webassembly port of tesseract. The maintainer stopped working on PDF processing in this app and will start working on separate app for pdf handling." : "# Descrizione\n\nNextcloud OCR (riconoscimento ottico dei caratteri) elabora le immagini con tesseract-js e porta la capacità OCR nel tuo Nextcloud.\nL'applicazione utilizza tesseract-js nel browser per elaborare le immagini (png, jpeg, tiff, bmp) e salva il file PDF risultante nella cartella di origine in Nextcloud. Ciò ti consente, ad esempio, di effettuare ricerche al suo interno.\n\n## Prerequisiti, requisiti e dipendenze\nL'applicazione OCR ha alcuni prerequisiti:\n\n- [Nextcloud 16 e versioni successive](https://nextcloud.com/)\n\n- Supportata solo sui browser web moderni più recenti (Chrome, Edge, Firefox, Opera, Safari*)\n\n- Tesseract traineddata richiede circa 200 MB di spazio sul tuo server (sarà installato automaticamente).\n\n\n_* Su Safari attualmente c'è un problema con il [Content-Security-Policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP), che richiede che un amministratore imposti 'script-src' su 'unsafe-eval' affinché l'applicazione funzioni correttamente. Poiché ciò è abbastanza insicuro, l'applicazione stessa non lo imposta e raccomanda di decidere a tuo rischio (assicurati di sapere cosa è CSP e cosa causa ad esempio unsafe-eval)._\n\n## Installazione\nInstalla l'applicazione dal [Nextcloud App Store](https://apps.nextcloud.com/apps/ocr) o scarica il pacchetto di distribuzione da github (**NON** i sorgenti) e posiziona il contenuto in **nextcloud/apps/ocr/**.\n\n## Liberatoria\nIl software è distribuito \"COME È\", SENZA GARANZIE O CONDIZIONI DI ALCUN TIPO, espresse o implicite.\n\n## Nota\nLa versione 3 e le versioni precedenti non sono più supportate/mantenute dall'autore. Quindi, per l'elaborazione asincrona in background, crea un fork del repository e utilizza il ramo \"not-maintained\" per lavorare su miglioramenti. L'autore non è stato in grado di supportarlo a causa dell'impegno eccessivo.\nInoltre, questo progetto si basa su una versione di tesseract in webassembly. Il manutentore ha smesso di lavorare sulla gestione dei PDF in questa applicazione e inizierà a lavorare su un'applicazione separata per la gestione dei PDF.",
    "Target file already exists:" : "Il file di destinazione esiste già",
    "An unexpected error occurred during the load of your favorite languages. No language will be set instead." : "Si è verificato un errore inatteso durante il caricamento delle tue lingue preferite. Nessuna lingua sarà impostata al suo posto.",
    "An unexpected error occurred during the upload of the processed file." : "Si è verificato un errore inatteso durante il caricamento del file elaborato.",
    "An unexpected error occurred during the deletion of the original file." : "Si è verificato un errore inatteso durante l'eliminazione del file originale.",
    "An unexpected error occurred during Tesseract processing." : "Si è verificato un errore inatteso durante l'elaborazione di Tesseract.",
    ...
    "_%n file is being processed:_::_%n files are being processed:_" : ["%n file è stato elaborato:","%n file sono stati elaborati:","%n file sono stati elaborati:"],
    "_Delete original file (image)_::_Delete original files (images)_" : ["Elimina il file originale (immagine)","Elimina i file originali (immagini)","Elimina i file originali (immagini)"],
